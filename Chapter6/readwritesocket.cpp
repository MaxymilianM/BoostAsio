...
// g++ -ansi -std=c++11 readwritesocket.cpp -o readwritesocket -I /usr/local/include -L /usr/local/lib/ -lboost_system -lboost_thread -lpthread
...
struct ClientContext : public boost::enable_shared_from_this<ClientContext> {
  ...
  // The boost::asio::async_write namespace writes the socket and invokes the OnSend() function handler.
  // Then, it erases the buffer and sends the next pending data if there is any. Now, every time we press any key in the telnet window,
  // it will display what we have typed because the readwritesocket project sends back what we type to the telnet window.
  ...
  // In the Send() function, we input an array of characters and their length.
  // Before the function sends the array of characters, it has to check whether or not
  // the m_send_buffer parameter is empty. The sending process can only occur if the buffer is not empty.
  ...
  // This function handler will format the received data to hexadecimal formatting.
  ...
  // This function will call the async_read_some() function to receive the set of data.
  void Recv() {
    m_socket.async_read_some(boost::asio::buffer(&m_recv_buffer[m_recv_buffer_index],
                                                  m_recv_buffer.size() - m_recv_buffer_index),
                             boost::bind(&ClientContext::OnRecv,
                                         shared_from_this(),
                                         std::placeholders::_1,
                                         std::placeholders::_2));
  }
...
int main(void) {
  ...
  try {
    ...
    acceptor->async_accept(client->m_socket, boost::bind(OnAccept, std::placeholders::_1, client));
