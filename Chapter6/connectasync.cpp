...
// g++ -ansi -std=c++11 connectasync.cpp -o connectasync -I /usr/local/include -L /usr/local/lib/ -lboost_system -lboost_thread -lpthread
...
int main(void) {
  ...
  // while the handler is waiting to be called, we have to use the boost::shared_ptr namespace.
  ...
  try {
    ...
    sckt->async_connect(endpoint, boost::bind(OnConnect, std::placeholders::_1));
  }
  catch(std::exception &ex) {
    global_stream_lock.lock();
    std::cout << "Message: " << ex.what() << ".\n";
