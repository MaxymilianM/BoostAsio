...
int main()
{
  ...
  try {
    ...
    // Note that in step 3, we don't open the socket when we create it.
    // This is because we don't know the version of IP addresses to which the provided DNS name will resolve.
    // The asio::connect() function opens the socket before connecting it to each endpoint specifying proper protocol object
    // and closes it if the connection fails.
