...
int main()
{
  ...
  // Because the service is specified as a protocol port number (in our case, 3333) and not as a service name (for example, HTTP, FTP, and so on),
  // we passed the asio::ip::tcp::resolver::query::numeric_service flag to explicitly inform the query object about that,
  // so that it properly parses the port number value.
  ...
  // This object represents a query to the DNS.
