...
1. In a client application, allocate, open, and connect an active TCP socket. In a server application,
obtain a connected active TCP socket by accepting a connection request using an acceptor socket.
...
std::string readFromSocketEnhanced(asio::ip::tcp::socket& sock) {
  ...
  // In the preceding sample, a call to the asio::read() function will block the thread of execution
  // until exactly 7 bytes are read or an error occurs.
