...
// g++ -ansi -std=c++11 asyncClientTCP.cpp -o asyncClientTCP -I /usr/local/include -L /usr/local/lib/ -lboost_system -lboost_thread -lpthread -lboost_chrono
...
struct Session {
  ...
  system::error_code m_ec; // Contains the description of an error if one occurs during the request life cycle.
...
public:
   ...
   void close() {
        ...
        // Wait for the I/O thread to exit for single thread.
        // m_thread->join();

        // Waiting for the I/O threads to exit for multiple threads.
...
int main()
{
  try {
    ...
    client.emulateLongComputationOp(10, "127.0.0.1", 4444, handler, 1);
    // Then does nothing for 5 seconds.
    std::this_thread::sleep_for(std::chrono::seconds(5));
    // Then initiates another request with id 2.
    client.emulateLongComputationOp(11, "127.0.0.1", 4445, handler, 2);
    ...
    client.emulateLongComputationOp(12, "127.0.0.1", 4446, handler, 3);
